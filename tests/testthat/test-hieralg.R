...
test_that("identifies changepoints differently, if we take into account the different algorithm implementation", {
  ...
  results <- hieralg(data_2, penalty = function(X) (0.2 * 3^ncol(X)) * log(nrow(X)))
  expect_equal(results$segments, list(1:7, 8:10, 11:15))
})

test_that("can be called with segment", {
  results <- segment(data_1, penalty = function(X) (0.1 * 2^ncol(X)) * log(nrow(X)), algorithm = "hierarchical")
  expect_equal(results$segments, list(1:5, 6:10, 11:15))

  results <- segment(data_2, penalty = function(X) (0.2 * 3^ncol(X)) * log(nrow(X)), algorithm = "hieralg")
  expect_equal(results$segments, list(1:7, 8:10, 11:15))
})
...
test_that("has detailed changepoints in the result set", {
  ...
  expect_equal(results$detailed_changepoints, list(
    list(changepoint = 5, gamma = 18.38363),
    list(changepoint = 10, gamma = 6.702574)
  ), tolerance = 0.001)
})
