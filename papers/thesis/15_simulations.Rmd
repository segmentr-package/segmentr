...
likelihood [@multivariate-likelihood], for which an implementation is provided in
the `segmentr` package as the `multivariate()` function, it's possible to identify
the set of $N_b$ changepoints
...
To ilustrate, consider the following code, which defines $X_1, ..., X_6$ and $C_1, ..., C_{15}$ as specified in \@ref(eq:example-columns), and alo defines the matrix $D$ as the column binding of the vectors $C_1, ..., C_{15}$.
...
C14 <- X5 + X6
C15 <- X5
D <- cbind(C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, C11, C12, C13, C14, C15)
head(D)
```
...
segments of homozigosity in the data. It's possible to use `segmentr` to find segments
...
Therefore, it's possible to have `segmentr` find the changepoints in the data using
the adequate functions. It's necessary to penlize the likelihood function because
...
segment(D, likelihood = function(X) mean_likelihood(X) - 1, algorithm = "hieralg")
```
