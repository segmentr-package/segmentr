...
Consider a matrix ${\bf X } \in \R^{n\times p}$ of  real data (discrete or
...
and therefore the rows of ${\bf X}$ are independent.

For $j\in \{1,\dotsc, p\}$  we denote by ${\bf X}^{j}$ the $j$th column of ${\bf
X}$ and given an interval of contiguous variables $I\subset\{1,\dotsc, p\}$ we
denote by ${\bf X}_I$ the sub-matrix with columns corresponding to elements of
$I$.
...
distribution of ${\bf X}$ is `homogeneous` inside each block $I$, that is
${\bf X}^j_i \sim f_{I(j)} \quad\text{ for all }i=1,\dotsc, n$, where $I(j)$ is
...
Observe that we are not assuming independence between the columns of ${\bf X}$,
this is an additional assumption that depends on the specific application.
Moreover, the independence between the rows of ${\bf X}$ could also be ignored,
