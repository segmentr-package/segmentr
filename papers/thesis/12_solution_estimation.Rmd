...
change points is stored in $C$. The solution will be exact precisely because it
analyses all the possible combinations. However, that has a time complexity of
$O(m^2)$ in the Big-O notation, for a number of column $m$ in the data set.
Because the number of columns in a data segmentation problems can be very
large, computation time can be very prohibitive. For those situations
approximate solutions are described.

## Hierarchical Algorithm

To try to simplify the search path of the algorithm, [@base-paper] also
...
points $C$ with time complexity of $O(m\log(m))$, for a number of columns $m$
...
it will switch to the exact algorithm. The procedure would be as follow:
...
of the conditional step in the beginning of the procedure that tests whether or
not to use the hybrid algorithm. As it will be shown in later chapters, the
