...
In order to exemplify how Segmentr is used in real situations, an example on weather
...
In order to grasp the behavior of the weather data, in Figure
...
lines are placed in Figure \@ref(fig:berlin-ideal). The indices picked to represent each segment are available in Table \@ref(tab:berlin-ideal-table).
...
Equation \@ref(eq:linear-regression-likelihood), $L_{lm}$ is the linear regression likelihood function, $X$ is the set of points
...
(\#eq:linear-regression-likelihood)
\end{equation}

A likelihood argument for a `segment()` call requires a function which accepts
...
\@ref(eq:linear-regression-likelihood) is implemented as an R function called
`lm_likelihood`, such that it obeys the argument contract by taking a matrix as
...
behaves with the `berlin` data set, sample likelihoods a small, a medium,
and a large segment are provided in \@ref(tab:likelihood-samples).

```{r likelihood-samples}
...
tibble(
  `Small Size`=lm_likelihood(berlin[, 2:3]),
  `Medium Size`=lm_likelihood(berlin[, 1:150]),
  `Large Size`=lm_likelihood(berlin)
) %>%
  kable(
...
With the likelihood function defined, it can now be applied to `segment()` in
...
$O(n \log(n))$. We point the hierarchical algorithm, generally suitable for the
...
\@ref(tab:results-non-penalized), and they are also plotted togetheer with the
weather data in Figure \@ref(fig:berlin-non-penalized).

```{r results-non-penalized}
results <- segment(
  berlin,
...
```{r berlin-non-penalized, fig.cap = "Berlin weather data segmented using a non-penalized likelihood function"}
plot_results(results, berlin)
```

In Figure \@ref(fig:berlin-non-penalized), it's possible see many very short
segments, and a very large last segment. This is a result of the algorithm
used, as well as the fact the `lm_likelihood` function tends to favor very
...
segment's length $l$, has the property that, for parametrization values
$C_1 > 0$, $s_1 > 0$, $C_2 > 0$ and $s_2 > 0$, the penalty is high for values of $l$
neighboring $0$, as well as values of $l$ the total length $L$ of the data set.
...
$\mu_s$ be the average likelihood for the sampled small segments and $\mu_b$ be
...
So, a penalized likelihood version of the function is created with
`auto_penalize()` and then used with `segment()`. The results of the
segmentation can be seen in Table \@ref(tab:results-auto-penalized), and it's
possible to the segments with the weather data in Figure
\@ref(fig:berlin-auto-penalized).

```{r results-auto-penalized}
penalized_likelihood <- auto_penalize(berlin,
                                      lm_likelihood)
results <- segment(
  berlin,
...
```{r berlin-auto-penalized, fig.cap = "Berlin weather data segmented using an auto-penalized likelihood function"}
plot_results(results, berlin)
```

The function above found two segments, the last of which was still quite large, which suggests the
`big_segment_penalty` argument needs to be increased to avoid that type of segment in the function.
So, by increasing it's value, from the default of $10$, all the way up to $100$, we run the results
again with the `segment()` function. The results can be seen in \@ref(tab:results-adjusted-penalized),
and it's possible to the segments with the weather data in Figure \@ref(fig:berlin-adjusted-penalized).

```{r results-adjusted-penalized}
penalized_likelihood <- auto_penalize(
  berlin,
  lm_likelihood,
  big_segment_penalty = 1000
)
results_berlin <- segment(
  berlin,
  likelihood = penalized_likelihood,
  algorithm = "hierarchical"
)
...
```{r berlin-adjusted-penalized, fig.cap="Berlin weather data segmented using an adjusted penalized likelihood function"}
plot_results(results_berlin, berlin)
```

Even with the new adjusted `penalized_likelihood`, the segments seen in Figure
\@ref(fig:berlin-adjusted-penalized) aren't similir to what we are looking for as
shown in Figure \@ref(fig:berlin-ideal). The reason behind this, as discussed
...
tolerable by reducing the granularity of the data, getting the monthly averages for each of the the
...
A new `penalized_function` is then built and applied to the monthly data set
using `segment()`, with the results being shown in Table
\@ref(tab:downres-auto-penalized), and with we also plot it with weather data
in Figure \@ref(fig:downsampled-auto-penalized).

```{r downres-auto-penalized}
penalized_likelihood <- auto_penalize(
  monthly_berlin,
  lm_likelihood,
...
```{r downsampled-auto-penalized, fig.cap="Downsampled Berlin monthly weather data segmented using an auto penalized likelihood function"}
...
Take notice the picking of the likelihood just have be able to rank segments in
...
the implementation of a `rsquared_likelihood`, which fits a linear regression
...
function is applied agains different segment sizes of the `berlin` data set,
analogous to Table \@ref(tab:likelihood-samples), and the results are shown in
...
previous case, the new `rsquared_likelihood` has the highest values for small
...
```{r rsquaredplot-auto-penalized, fig.cap="Berlin daily weather data segmented using an auto-penalized R-squared likelihood function"}
plot_results(results, berlin)
```

The default penalized `rsquared_likelihood` split the data set in three
segments, roughly the same size each. Since the penalty model applied by
`auto_penalize`, it applies the least penalty for values segments closer to
about half the total length, so increasing the `big_penalty_segment` will have
...
```{r rsquaredplot-adjusted-penalized, fig.cap="Berlin daily weather data segmented using an adjusted penalized R-squared likelihood function"}
plot_results(results, berlin)
```
...
```{r subberlinplot-penalized, fig.cap="Subset of Berlin daily weather data segmented using an auto-penalized R-squared likelihood function"}
plot_results(results, sub_berlin)
```

In Figure \@ref(fig:subberlinplot-penalized), we can clearly see the segments
