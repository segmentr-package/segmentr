...
linear regression fit, the method's standard log-likelihood function (i.e.
the negative of the squared sum of residuals) is a good candidate for our segment likelihood function.
However, the sum of
residuals tend to increase with the number of points in a segment. Instead, we pick the negative mean of the
...
behaves with the `berlin` data set, sample likelihoods for a small, a medium, and a large segment are provided in \@ref(tab:likelihood-samples).
...
With the likelihood function defined, it can now be applied to `segment()` to get the segments for the data set. Since the time complexity of the
...
multivariate likelihood) assumes the segments' likelihoods to be structured hierarchically, with a combination of two neighboring segments being selected as an intermediate step before evaluating the ideal change
...
the average likelihood for the sample's big segments. The relationship between
...
likelihood function in Figure \@ref(fig:berlin-auto-penalized) found by the new
penalized likelihood, though it's also over segmenting some downward trends. It
...
`penalized_likelihood`, the segments seen in Figure
...
Notice the only requirement on the likelihood function is for it to be able to rank segments in
...
the default `auto_penalize` funtion has the lowest penalty for segments of sizes closer to
...
has an unattractive likelihood, due to the presence of alternating trends.
