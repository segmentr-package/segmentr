...
\@ref(tab:resultsnonpenalized), and they are also plotted togetheer with the
...
again with the `segment()` function. The results can be seen in \@ref(tab:resultsadjustedpenalized),
...
In order to make the solution comparison clearer, it's possible to
rescale the `exact` solution to the monthly sampled dataset to the
daily sampled dataset by applying a linear trasformation of
...
```{r plot-results-rescaled, fig.cap="Rescaled segments estimated for a downsampled monthly weather data of the Berlin data set using an auto penalized likelihood function"}
