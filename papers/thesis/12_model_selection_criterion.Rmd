...
\hat S \;=\; \underset{S}{\arg\max}\; \Bigl\{ \sum_{I\in S} ( \log \hat\P ({\bf X}_I) -  \text{pen}(I)) \Bigr\}\,,
\end{equation}
...
$\hat s \;=\; \underset{s}{\arg\min} \sum_{s} (f({\bf X}_s) + h(s))$,
where ${\bf X}\in \R^{n\times p}$ and  $s$ is a set of `segments` over
...
The function $c({\bf X},s)=\sum_{s} f({\bf X}_s)+h(s)$ is a `cost` function
